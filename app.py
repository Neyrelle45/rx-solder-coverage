...
if model_file:
    ...
    if rx_upload and mask_upload:
        ...
        m_black = cv2.resize(((b_c < 50) & (g_c < 50) & (r_c < 50) & (m_green > 0)).astype(np.uint8), (W, H), interpolation=cv2.INTER_NEAREST)
        ...
        for v in top_5:
            cv2.drawContours(overlay, [v['poly']], -1, [0, 255, 255], 2) # Cyan
        ...
        with c_res:
            ...
            if st.button("📥 Archiver l'analyse", use_container_width=True):
                ...
                st.toast("Analyse archivée")

        with c_img:
            st.image(overlay, caption="Résultat Visuel (Cyan = Voids Internes)", use_container_width=True)
...
if st.session_state.history:
    ...
    st.download_button("🎁 Télécharger ZIP Complet", buf.getvalue(), "session_export.zip", "application/zip")
