...
if model_file:
    ...
    if rx_upload and mask_upload:
        ...
        # Zone de sécurité (pour éviter les bords du masque)
        z_safe = cv2.erode(z_adj, np.ones((3,3), np.uint8))
        ...
        # Identification du VOID MAXIMUM
        ...
        for c in cnts:
            ...
            if area < 5.0: continue # Ignorer le bruit
            
            c_m = np.zeros((H, W), dtype=np.uint8)
            cv2.drawContours(c_m, [c], -1, 255, -1)
            
            # CRITÈRES DE VALIDITÉ :
            # 1. Ne doit pas contenir de via (trou noir)
            # 2. Doit être majoritairement dans la zone "Safe" (pas sur le bord)
            if not np.any((c_m > 0) & (hol_adj > 0)):
                if np.sum((c_m > 0) & (z_safe > 0)) / np.sum(c_m > 0) > 0.8:
        ...
        if max_void_poly is not None:
            cv2.drawContours(overlay, [max_void_poly], -1, [0, 255, 255], 3) # Cyan épais
        ...
        with c_res:
            ...
            st.metric("Void Maximum", f"{max_void_pct:.3f} %")
        ...
        with c_img:
            st.image(overlay, caption="Cyan = Plus gros void détecté", use_container_width=True)

# --- RAPPORT ---
if st.session_state.history:
    st.divider()
    st.subheader("📊 Rapport de Session")
    df_csv = pd.DataFrame(st.session_state.history).drop(columns=['img_bytes'])
    ...
    with zipfile.ZipFile(buf, "w") as z:
        ...
        for i, item in enumerate(st.session_state.history):
            z.writestr(f"img_{i+1}.jpg", item['img_bytes'])
    st.download_button("🎁 Télécharger ZIP", buf.getvalue(), "rapport_rx.zip", "application/zip")
